...
import { getProgrammesWithNombreProj } from "../utils/api"; 
...
const ProgrammesWithNombreProj = ({ isDashboard = false }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getProgrammesWithNombreProj();
        const mappedData = response.map((item) => ({
          "Intitule programme": item.intitule_programme, 
          "Nombre de projets": item.nombre_projets, 
        }));
        setData(mappedData);
      } catch (error) {
        console.log(error);
  ...
  return (
    <ResponsiveBar
      ...
      axisBottom={{
        ...
        tickPadding: 5,
        tickRotation: 0,
        legend: isDashboard ? undefined : "Nombre de projets", // Adjust the legend text if used in a dashboard
...
export default ProgrammesWithNombreProj;
